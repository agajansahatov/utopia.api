...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `copy_tables` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `copy_tables`()
BEGIN
  ...
  read_loop: LOOP
    ...
    IF done THEN
      LEAVE read_loop;
    END IF;
    SET @stmt = CONCAT('CREATE TABLE ', @NEW_DB, '.', table_name, ' LIKE ', @OLD_DB, '.', table_name, ';');
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `products_summary_view` AS select `p`.`id` AS `id`,`p`.`title` AS `title`,`p`.`original_price` AS `original_price`,`p`.`sales_price` AS `sales_price`,`p`.`number_in_stock` AS `number_in_stock`,concat(left(`p`.`description`,500),' ...') AS `description`,`p`.`date` AS `date`,`m`.`name` AS `main_media` from (`products` `p` left join `medias` `m` on(((`p`.`id` = `m`.`product_id`) and (`m`.`id` = (select min(`m1`.`id`) from `medias` `m1` where ((`m1`.`product_id` = `p`.`id`) and (`m1`.`is_main` = 1)) group by `m1`.`product_id`))))) order by `p`.`id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
